...
class UserActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(user == null){
            ...
            if (isConnectable(user!!)) {
                ...
                connectivityStatus.setTextColor(resources.getColor(R.color.green))
                connectButton.isEnabled = true
            } else {
                connectivityStatus.text = Resources.getSystem().getString(R.string.not_ready)
                connectivityStatus.setTextColor(resources.getColor(R.color.red))
    ...
    private fun isConnectable(user : User): Boolean {
    ...
    fun updateUI(apiUser : User){
        Log.d(tag, apiUser.toString())
        //for each user in userList we add a card to CardAdapter
        runOnUiThread {
